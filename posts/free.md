...
This relies on the [free](https://hackage.haskell.org/package/free) package which defines standard *free* Constructions for `Applicative` and `Monad`, and *cofree*  for `Comonads`.
...
Then we can define our complex interpreter and what interpretation means in the context of this composite. `coiter` is a function from the [`Cofree`](https://hackage.haskell.org/package/free-4.12.1/docs/Control-Comonad-Cofree.html) module that "lifts" computation in a Functor into a `Cofree` monad, starting from a seed value.
